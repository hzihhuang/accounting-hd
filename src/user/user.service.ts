...
export class UserService {
  ...
  async updatePassword(userId: number, updatePasswordDto: UpdatePasswordDto) {
    ...
    const isPasswordValid = await bcrypt.compare(updatePasswordDto.newPassword, updatePasswordDto.oldPassword);
  ...
  async updateAvatar(userId: number, updateAvatar: UpdateAvatarDto) {
    const user = await this.userRepository.findOne({ where: { id: userId } });
    if (!user) return { code: 404, message: '用户不存在' }
    await this.userRepository.update(user.id, { avatar: updateAvatar.newAvatar })
